...
class TestMoveEmails:
    ...
    async def test_move_emails_single_uid_success(self, tools):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids="42", target_folder="Archive", folder="INBOX")
    ...
    async def test_move_emails_multiple_uids(self, tools):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids=["10", "11"], target_folder="Projects", folder="INBOX")
    ...
    async def test_move_emails_disabled_by_default(self, tools):
        ...
        result = await tools.move_emails(uids=["1", "2"], target_folder="Projects", folder="INBOX")
    ...
    async def test_move_emails_no_credentials(self):
        ...
        result = await t.move_emails(uids=["1"], target_folder="Projects", folder="INBOX")
    ...
    async def test_move_emails_imap_error(self, tools):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids=["42"], target_folder="Projects", folder="INBOX")
    ...
    async def test_move_emails_partial_failure(self, tools):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids=["1", "5"], target_folder="Projects", folder="INBOX")
        assert "1" in result
        assert "5" in result
    ...
    async def test_move_emails_generic_exception(self):
        ...
        with patch_imap_mailbox_attr("_IMAP_EXCEPTION", CustomIMAPError):
            ...
            t.valves.password = "testpass"
            t.valves.allow_move = True
            result = await t.move_emails(uids=["42"], target_folder="Projects", folder="INBOX")
    ...
    async def test_move_emails_to_nested_folder(self, tools):
        ...
        mock_server = _make_mock_server([(raw, "1")])
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids="1", target_folder="Projects/Finance/Invoices", folder="INBOX")
    ...
    async def test_move_emails_comma_separated(self, tools):
        ...
        with patch("imaplib.IMAP4_SSL", return_value=mock_server):
            result = await tools.move_emails(uids="10,11", target_folder="Archive", folder="INBOX")
        assert "10" in result
        assert "11" in result
        assert "2 email(s)" in result
